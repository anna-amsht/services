...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody RegisterRequestDto registerRequest) {
        ...
        try {
            Map<String, Object> internalUserRequest = new HashMap<>();
            internalUserRequest.put("id", user.getId()); // ID generated by AuthService
            internalUserRequest.put("email", registerRequest.getEmail());

            String userServiceUrl = "http://localhost:8082/api/v1/internal/users";
            logger.info("Calling UserService internal endpoint to create user profile with ID: {}", user.getId());
            logger.info("Using internal token: {}", internalToken);
            
            org.springframework.http.HttpHeaders headers = new org.springframework.http.HttpHeaders();
            headers.set("X-Internal-Token", internalToken);
            org.springframework.http.HttpEntity<Map<String, Object>> entity = 
                new org.springframework.http.HttpEntity<>(internalUserRequest, headers);
                
            ResponseEntity<String> response = restTemplate.postForEntity(userServiceUrl, entity, String.class);

            if (!response.getStatusCode().is2xxSuccessful()) {
                logger.error("UserService returned error status: {}, body: {}", response.getStatusCode(), response.getBody());
                userDao.delete(user.getId());
                return ResponseEntity.status(response.getStatusCode()).body("Failed to create user profile in UserService: " + response.getBody());
            }
            logger.info("Successfully created user profile in UserService with ID: {}", user.getId());
        } catch (Exception e) {
            logger.error("UserService is unavailable: {}", e.getMessage(), e);
            userDao.delete(user.getId());
            return ResponseEntity.status(503).body("UserService is unavailable: " + e.getMessage());
        }
